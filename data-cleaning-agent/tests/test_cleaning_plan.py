...
def test_default_plan_seeds_coerce_columns_from_summary(mixed_df, summary) -> None:
    plan = default_plan_from_summary(
        summary, user_instructions=None, row_id_col="__agent_row_id__"
    )
...
@pytest.mark.unit
def test_default_plan_always_protects_row_id(mixed_df, summary) -> None:
    plan = default_plan_from_summary(
        summary, user_instructions="protect country", row_id_col="__agent_row_id__"
    )
    assert "__agent_row_id__" in plan.protected_columns
...
def test_cleaning_plan_rejects_unknown_skip_step() -> None:
    with pytest.raises(ValueError, match="unknown skip_steps"):
        CleaningPlan.model_validate({"skip_steps": ["not_a_step"]})
