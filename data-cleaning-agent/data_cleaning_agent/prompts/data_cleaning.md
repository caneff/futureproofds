...
then omit only those operations while keeping the rest of the pipeline coherent.
This prompt emits **Python only**—implement the pipeline faithfully in code.
...
- Never drop or destructively transform any column whose **normalized** name
  appears **as literal text** in the **User Instructions** section at the end of
  this prompt, or the synthetic row id column (``__agent_row_id__``) described
  above. Treat those as protected. A column that appears **only** in Dataset
  Summary is **not** protected merely because it is listed there.
...
7. Drop columns that are constant (one unique non-null value) or 100% NaN.
   **After steps 3 and 7, treat the set `df.columns` as authoritative.** Any column
   ...
   no `df.drop(columns=[...])` that lists it, and no loops or lists for steps 8–11
   built from the original `source_df` names or Dataset Summary alone. For step
   8 especially, build candidate column lists only from columns **still on
   ...
   name in step 8—**forbidden**; that causes `KeyError` at runtime.
8. Impute missing values (always assign back to ``df[col]``) using judgment that
   fits User Instructions and each column’s role; there is **no** built-in list of
   columns exempt from fills—decide yourself which columns should receive mean,
   median, mode, or no fill.
   - **Numeric columns:** ``skew_val = df[col].skew()`` (a SCALAR float). Use the
     built-in ``abs(skew_val)``; NEVER call ``.abs()`` on the scalar. If
     ``abs(skew_val) > 1`` use
     ``df[col] = df[col].fillna(df[col].median())``, else
     ``df[col] = df[col].fillna(df[col].mean())`` when you choose to impute that column.
   - **String / object columns** (including nullable string dtypes): when you use
     a mode fill, **recompute** missing share on the **current** ``df`` immediately
     beforehand (after steps 4–5; same NA / empty-stripped / placeholder semantics as
     step 3). Prefer leaving NaN where a fill would invent meaning. Do **not** use
...
10. Drop exact duplicate rows: df.drop_duplicates().
...
Return **only** one fenced block (no preamble, no trailing commentary):
